...
Use cases:
------------
This Neural Network module can be used to train models for `Multi-class classification`.    
...
class NN_Layer():
    ...
    def __init__(self, layer: str ,units: int, activation_func: str= 'linear') -> None:
        ...
        match activation_func:
            ...
            case 'softmax':
                self.g = self.Softmax.g
                self.dj_dz = self.Softmax.dj_dz
                self.cost = self.Softmax.cost
    ...
    class Softmax:
        ...
        ##
        @staticmethod
        def dj_dz(y: np.array, y_hat: np.array) ->np.array:
            return y_hat - y
...
class NN():
    ...
    def train(self, features: np.array, labels: np.array, alpha: float= .001, n_iterations:int=10000, e:float=1e-4, print_cost=True, print_each:int=1000):
        ...
        for i, hidden_layer in enumerate(self.hidden_layers):
            ...
            hidden_layer.b = np.zeros([1, hidden_layer.units])               # b.shape(1, Nr_neurons)
        ...
        for _ in range(0, self.n_iterations):
            ...
            #02.02.01 At the Last Layer(Softmax Layer):
            self.cost = self.hidden_layers[-1].cost(y=self.labels, y_hat=self.hidden_layers[-1].a, m= self.m)
            if print_cost and _%print_each == 0:
                print(self.cost)
            if abs(self.cost - cost_prev) <= e:
                if print_cost:
            ...
            dj = self.hidden_layers[-1].dj_dz(y= self.labels, y_hat= self.hidden_layers[-1].a)        
            
            #dj = dj*da_dz
            self.hidden_layers[-1].dj_db = np.sum(dj, axis=0).reshape(1,-1) # shape: (1, Nr_neurons)  
